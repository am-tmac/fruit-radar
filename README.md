...
离线验收（不运行 `live` feature 或 `--ignored`，不访问真实 Apple、不发送真实通知、不加购/下单）：

```bash
cargo test --workspace && pnpm test && pnpm exec tsc --noEmit && pnpm tauri build --bundles app
codesign --verify --deep --strict target/release/bundle/macos/水果雷达.app
```

任何失败停止后续构建/部署，不用管道隐藏退出码。打包不代表安装或重启。

- 自动加购按地区 + SKU 在本次应用运行中最多尝试一次，总计最多 32 项；待执行票据 30 秒过期，暂停或修改设置使旧票据失效。失败后避免盲目重复加购，可由用户手动检查购物袋再操作。已启动的购买流程不强杀用户浏览器。
- 加购前仅允许空白、Apple 商品或购物袋页面，结账、订单、登录或未知页面拒绝自动导航；程序不提交订单、不处理支付。订单监听单实例，同一订单本次运行最多发送三次（仅失败重试），重启后不保留订单去重记录。
- 查询会话与 `3d7c56e` 一致：每次建立独立临时 profile，403/541 或 CDP 传输失败销毁当前会话，下次查询重建；429 保留会话。无持久 `chrome-query`、迁移、generation 或启动目录扫描，不读取或删除以前遗留的目录。仅 RAII 回收本次创建的进程和临时目录；541 原因仍需现场验证，不归因于冷会话或 IP。
- 旧临时 profile 无可信所有者证据时全部保留，不以调试端口不可达授权删除。
- Apple 返回 541 后，后续轮次重试可能恢复；这不代表 541 已解决，也不能据此判断其原因。应用启动后必须手动点击「开始监控」。
- 活动日志入口隐藏 URL，每行 UTF-8 限长 8 KiB，文件上限 512 KiB；独立文件锁覆盖追加和滚动替换。日志不是秘密存储，不要主动粘贴密钥或个人信息。
- 启动始终等待手动开始，即使旧恢复标记为运行中也不自动查询；启停仍串行化并按 actor 实际状态记录标记，保留独立临时文件原子替换。启动期间前端事件暂存、快照后重放不变。离线 RecordingFetcher 对照仅证明启动时序差异和相同查询入参，不证明自动恢复导致 541，也不证明 541 已修复。
...
- **到货后打开**：可以选择「不自动打开」「购物袋」或「商品详情」。这些选项只负责打开页面，不会自动加入购物袋、预留库存或下单。需要程序替你加购，请用下面的独立开关。开启自动加购后，这一项不再另外调用系统浏览器。

持续有货时，每一轮都会再次执行已经开启的提醒。如果不想浏览器反复出现新页面，请把「到货后打开」设为「不自动打开」。

### 有货时自动加入购物袋（可选）

水果雷达可以在检测到有货的那一刻，替你把商品放进 Apple 的购物袋 —— **只到购物袋为止，不结账、不付款**。

开启前先做一次准备：

1. 在右侧「监控设置」里打开 **有货时自动加入购物袋**。
2. 点 **打开买家窗口**：会启动一个专门用于加购的浏览器窗口。
3. 在这个窗口里**登录你的 Apple 账号**。这一步只能由你完成，程序不碰账号和密码。

为什么必须单独一个窗口：购物袋是按浏览器的登录状态（cookie）记的。用程序自己的临时窗口加购，你在自己的浏览器里看不到任何东西。所以自动加购固定使用这个「买家窗口」，登录状态会长期保留。

之后只要检测到有货，程序会在这个窗口里打开对应商品页、在换购问题里固定选**「不折抵换购」**（折抵要评估旧机、还要你确认估价，那是你自己该做的决定，程序不替你选）、按设置选择是否加入 AppleCare+、点击「添加到购物袋」，然后停在购物袋页面等你付款。**最后一步永远由你点。**

需要知道的几点：

- **结果会写进活动日志**：成功写「自动加购完成」，失败会说明卡在哪一步。
- **购买页上有两道前置问题**：先问折抵换购、再问 AppleCare，两题都答完，加购按钮才会可用。程序按上面的规则替你答。
- **未开售的商品页面上没有这个按钮**（例如预购开始前的新机），此时自动加购会失败并给出提示，属于正常现象。
- 监控列表每一行有购物袋图标，鼠标移到该行即可看到，可以**手动试一次**，用来在真正开售前验证整条链路。
- 想关掉这个窗口，用设置里的 **关闭** 按钮，或者直接关掉窗口。

### 两种到货方式，两条不同的收尾路径

**门店取货** —— 需要动手的步骤最多，也是本工具最想省掉的那几秒：

```
自动加购 → 你点结账 → 选「取货」→ 选零售店 → 「继续填写取货详情」
→ 点「填写到结账页」代填取货联系人 → 选付款方式 → 检查订单 → 立即下单
→ 手机收到「订单已创建」通知 → 点开进 Apple Store App 订单页付款
```

实测确认：**Apple 不会从账号预填取货联系人**（登录状态下五栏全空），所以这里
每次都要手打姓氏、名字、电子邮箱、手机号、身份证件后四位 —— 设置里填一次，
之后由程序代填。程序只填这五栏，不提交订单、不碰支付。

**线上（送货）** —— 不需要额外步骤：收货地址与付款方式 Apple 账号里已经存着，
所以程序把商品加进购物袋就够了：

```
自动加购 → 你点结账 → 用账号里已有的地址和付款方式 → 立即下单
```

下单成功后程序会盯住订单确认页，一旦发现就推一条带订单号的通知到手机；
通知里的 `/shop/order/list` 是 Apple Store App 接管的路径，点开会直接进 App 的订单页。

### 点通知会打开什么（实测 + Apple 官方规则）

**商品页链接会被 Apple Store app 接管，购物袋链接不会。**

依据是 Apple 自己的域名关联文件
（`https://www.apple.com/.well-known/apple-app-site-association`，中国大陆站同一份）：
其中只有一个 app 声明接管 apple.com 的链接 —— `com.apple.store.Jolly`，也就是
**Apple Store 那个购物 app**。它接管的路径里包含 `/shop/buy-*` 与 `/shop/product/*`，
**不含 `/shop/bag`**。少数参数（`?step=preauth|fork|upgrade`）被显式排除，仍走浏览器。

实测确认：

- 带**商品页**链接的推送 → **直接进入 Apple Store app 并打开该商品**
- 带商品页 + `?step=attach` 的推送 → 仍进 app 的**商品页**（参数被忽略）
- 带 `/shop/bag` 的推送 → 只能落到 Safari

即：**商品页是 Apple 允许的最深跳转**，没法用链接直达 app 内的购物袋或结账步骤。
Apple 刻意把结账留在自己的流程里（`?step=preauth|fork|upgrade` 被显式排除给浏览器，
购物袋路径干脆不被 app 接管）。

所以：

- 想「点通知直接进 app」→ 把「到货后打开」设为**商品详情**（或型号专属 Bark 指向商品页）
- 设成**购物袋** → 只能进 Safari
- **未上架的机型没有商品页**（例如预购开始前的新机），此时商品页链接会 404，只能进 Safari ——
  一旦开售、Apple 发布商品页，链接就会自动生效并跳进 app
- 最快的实际组合是：通知跳进 app 的商品页 **+ 桌面端已经替你加购** ——
  进 app 后切到「购物袋」标签即可结账（前提是账号购物袋能跨设备同步，见下）

### 尚未验证：账号购物袋是否跨设备同步

Apple 官方帮助写明登录后「可以保存购物袋直到准备好下订单」，但**网页端加的购物袋
在手机 app 里是否立刻可见**没有验证过。这一条决定两种工作流：

- 若能同步 → 桌面自动加购 + 手机点通知进 app 结账，全程闭环
- 若不能 → 通知只负责「叫醒」，实际结账在桌面买家窗口完成

验证方法：用监控列表里的购物袋图标真加一件，再到手机 app 的「购物袋」标签查看。
